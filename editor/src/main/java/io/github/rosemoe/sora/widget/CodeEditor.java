...
public class CodeEditor extends View implements ContentListener, Formatter.FormatResultReceiver, InlayHintRendererProvider {
    ...
    private boolean highlightHexColors = false;
    
    private boolean enableLongPressSelection = true;
    private boolean enableDoubleTapSelection = true;
    
    /**
 * @param enabled 
 */
public void setEnableLongPressSelection(boolean enabled) {
    this.enableLongPressSelection = enabled;
}

/**
 * @param enabled 
 */
public void setEnableDoubleTapSelection(boolean enabled) {
    this.enableDoubleTapSelection = enabled;
}

/**
 */
public boolean isLongPressSelectionEnabled() {
    return enableLongPressSelection;
}

/**
 */
public boolean isDoubleTapSelectionEnabled() {
    return enableDoubleTapSelection;
}

    public interface OnTextSelectStateChangeListener {
        void onTextSelectStateChanged(boolean isSelecting);
    }

    private final List<OnTextSelectStateChangeListener> textSelectStateListeners =
            new ArrayList<>();
    ...
    public void setHighlightHexColorsEnabled(boolean enabled) {
        this.highlightHexColors = enabled;
        invalidate();
    }

    public boolean isHighlightHexColorsEnabled() {
        return highlightHexColors;
    }
    ...
    protected void initialize(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        colorScheme = EditorColorScheme.getSystemThemeScheme(getContext());
    ...
    public void addOnTextSelectStateChangeListener(OnTextSelectStateChangeListener listener) {
        OnTextSelectStateChangeListener wrapped =
                (isSelecting) -> {
                    try {
                        listener.onTextSelectStateChanged(isSelecting);
                    } catch (Exception e) {
                        Log.e(
                                LOG_TAG,
                                "Listener crashed: " + listener.getClass().getSimpleName(),
                                e);
                    }
                };
        textSelectStateListeners.add(wrapped);
    }

    public void removeOnTextSelectStateChangeListener(OnTextSelectStateChangeListener listener) {
        textSelectStateListeners.remove(listener);
    }

    void notifyTextSelectStateChanged(boolean isSelecting) {
        for (OnTextSelectStateChangeListener listener : textSelectStateListeners) {
            try {
                listener.onTextSelectStateChanged(isSelecting);
            } catch (Exception e) {
                Log.e(LOG_TAG, "Listener error", e);
            }
        }
    }
     ...
     * Get selected text.
     *
     * @return The selected text. If there is no selection, an empty string is returned.
     */
    @NonNull
    ...
    private void checkTextSelectState() {
        if (cursor == null) {
            return;
        }

        boolean currentSelectionState = cursor.isSelected();

        if (currentSelectionState != lastSelectionState) {
            notifyTextSelectStateChanged(currentSelectionState);
            lastSelectionState = currentSelectionState;
        }
    }


    public interface TextActionWindowListener {
        void onTextActionWindowClick(int itemId);
    }

    private TextActionWindowListener textActionWindowListener;

    public void setTextActionWindowListener(TextActionWindowListener listener) {
        this.textActionWindowListener = listener;
    }

    public void onTextActionWindowClick(int id) {
        if (textActionWindowListener != null) {
            textActionWindowListener.onTextActionWindowClick(id);
        }
    }
