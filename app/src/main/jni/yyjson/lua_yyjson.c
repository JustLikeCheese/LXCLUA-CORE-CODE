...
static int lua_yyjson_parse_and_process_json(lua_State *L) {
    ...
    // 首先解析为不可变文档
    yyjson_read_err err;
    yyjson_doc *immutable_doc = yyjson_read_opts((char *)json_str, strlen(json_str), YYJSON_READ_INSITU, NULL, &err);
    if (!immutable_doc) {
        lua_pushnil(L);
        lua_pushstring(L, err.msg);
        return 2;
