...
LUALIB_API int luaopen_rawio(lua_State *L) {
    ...
    // 设置版本信息
    lua_pushstring(L, "V1Pro至尊版");
    lua_setfield(L, -2, "版本");
    
    // 设置作者信息
    lua_pushstring(L, "DifierLine");
    lua_setfield(L, -2, "作者");
    
    // 设置说明信息
    lua_pushstring(L, "file_read=ioread=read\nfile_tsread=iotsread=tsread\nfile_write=iowrite=write");
    lua_setfield(L, -2, "说明");
    
    // 设置群聊信息
    lua_pushstring(L, "QQ群聊:818459127");
    lua_setfield(L, -2, "星辰互联");
