...
static int l_ptr_new (lua_State *L) {
  void *p = lua_topointervalue(L, 1);
  if (p == NULL && !lua_ispointer(L, 1)) {
    return luaL_error(L, "expected pointer value");
  }
  lua_pushpointer(L, p);
  return 1;
}
...
static int l_ptr_addr (lua_State *L) {
  const void *p = lua_topointervalue(L, 1);
  if (p == NULL && !lua_ispointer(L, 1)) {
    return luaL_error(L, "expected pointer value");
  }
...
static int l_ptr_sub (lua_State *L) {
  const void *p1 = lua_topointervalue(L, 1);
  const void *p2 = lua_topointervalue(L, 2);
  if ((p1 == NULL && !lua_ispointer(L, 1)) || 
      (p2 == NULL && !lua_ispointer(L, 2))) {
    return luaL_error(L, "expected pointer values");
  }
  ptrdiff_t diff = (const char *)p1 - (const char *)p2;
...
static int l_ptr_write (lua_State *L) {
  void *p = (void *)lua_topointervalue(L, 1);
  if (p == NULL && !lua_ispointer(L, 1)) {
    return luaL_error(L, "expected pointer value");
  }
  const char *type = luaL_checkstring(L, 2);
