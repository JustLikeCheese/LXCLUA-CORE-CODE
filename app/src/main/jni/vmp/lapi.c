...
LUA_API int lua_ispointer (lua_State *L, int idx) {
  const TValue *o = index2value(L, idx);
  return ttispointer(o);
}
...
LUA_API void *lua_topointervalue (lua_State *L, int idx) {
  const TValue *o = index2value(L, idx);
  return ttispointer(o) ? ptrvalue(o) : NULL;
}
