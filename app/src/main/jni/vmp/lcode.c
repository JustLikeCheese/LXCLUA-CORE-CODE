...
void luaK_pipe (FuncState *fs, expdesc *e1, expdesc *e2) {
  ...
  if (e2->k == VNONRELOC) {
    ...
    /* 函数调用结果：u.info 是指令位置，需要从中提取目标寄存器(A) */
    func_reg = GETARG_A(fs->f->code[e2->u.info]);
  } else if (e2->k == VRELOC) {
    /* 闭包/可重定位结果：
     * luaK_exp2anyreg 处理 VRELOC 时会调用 luaK_exp2nextreg
     * 将闭包移动到 freereg-1 位置
     * 我们需要将闭包移动到 freereg+1 位置，避免与后续参数冲突
     */
    int closereg = fs->freereg - 1;
    if (closereg < 0) closereg = 0;
    /* 预留一个寄存器给参数，将闭包移动到 freereg+1 */
    luaK_reserveregs(fs, 1);
    luaK_codeABC(fs, OP_MOVE, fs->freereg, closereg, 0);
    /* 更新 e2 的信息 */
    e2->k = VNONRELOC;
  ...
  /* 保存参数所在寄存器 */
  int arg_reg = e1->u.info;
  
  /* 如果参数寄存器与函数寄存器冲突，需要先保存函数 */
  if (arg_reg == func_reg) {
    /* 将函数移动到 func_reg+1（临时保存） */
    luaK_codeABC(fs, OP_MOVE, func_reg + 1, func_reg, 0);
    func_reg = func_reg + 1;
  }
  /* 如果参数寄存器不在函数寄存器的下一个位置，移动它 */
  else if (arg_reg != func_reg + 1) {
    luaK_codeABC(fs, OP_MOVE, func_reg + 1, arg_reg, 0);
    arg_reg = func_reg + 1;
  }
  
  /* 生成函数调用指令：OP_CALL A B C */
  /* A = func_reg：函数寄存器，也是结果存储位置 */
  /* B = 2：有1个参数（寄存器A是函数，A+1是参数） */
  /* C = 2：期望1个返回值 */
