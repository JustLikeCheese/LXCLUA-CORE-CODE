...
/*
** 参数默认值信息结构
** 功能描述：存储函数参数的默认值，用于支持 function foo(a = 1) 语法糖
** 参数说明：
**   paramreg - 参数在寄存器中的位置
**   hasdefault - 是否有默认值
*/
#define MAX_PARAM_DEFAULTS 200

typedef struct ParamDefaultInfo {
  int paramreg;      /* 参数寄存器位置 */
  expdesc defexpr;   /* 默认值表达式 */
} ParamDefaultInfo;
...
/**
 * 解析函数参数列表（支持参数默认值）
 * 功能描述：解析函数参数，支持 name = expr 形式的默认值语法糖
 * 参数说明：
 *   ls - 词法状态
 *   defaults - 存储参数默认值信息的数组（输出参数）
 *   ndefaults - 有默认值的参数数量（输出参数）
 * 语法示例：function foo(a, b = 1, c = "hello")
 */
static void parlist_with_defaults (LexState *ls, ParamDefaultInfo *defaults, int *ndefaults) {
  /* parlist -> [ {NAME ['=' expr] ','} (NAME ['=' expr] | '...') ] */
  FuncState *fs = ls->fs;
  Proto *f = fs->f;
  int nparams = 0;
  int isvararg = 0;
  int baseReg = fs->nactvar;  /* 保存参数开始的基础寄存器位置 */
  *ndefaults = 0;
    ...
    do {
      switch (ls->t.token) {
        case TK_NAME: {
          ...
          /* 检查是否有默认值 */
          if (testnext(ls, '=')) {
            /* 保存参数寄存器位置（基础位置 + 当前参数索引） */
            defaults[*ndefaults].paramreg = baseReg + nparams;
            expr(ls, &defaults[*ndefaults].defexpr);
            (*ndefaults)++;
          }
          nparams++;
          break;
...
/**
 * 生成参数默认值检查代码
 * 功能描述：为有默认值的参数生成 if param == nil then param = default end 代码
 * 参数说明：
 *   ls - 词法状态
 *   defaults - 参数默认值信息数组
 *   ndefaults - 有默认值的参数数量
 */
static void gen_param_defaults (LexState *ls, ParamDefaultInfo *defaults, int ndefaults) {
  FuncState *fs = ls->fs;
  int i;
  for (i = 0; i < ndefaults; i++) {
    ...
    /* 获取参数变量 */
    param.k = VLOCAL;
    param.u.var.ridx = defaults[i].paramreg;
    param.u.var.vidx = defaults[i].paramreg;
    /* 生成 param == nil 的条件检查 */
    init_exp(&cond, VLOCAL, defaults[i].paramreg);
    luaK_goiftrue(fs, &cond);  /* 如果不为nil则跳过 */
    jf = cond.f;
    /* 生成赋值代码：param = default */
    luaK_exp2nextreg(fs, &defaults[i].defexpr);
    luaK_storevar(fs, &param, &defaults[i].defexpr);
    /* 修补跳转地址 */
...
static void body (LexState *ls, expdesc *e, int ismethod, int line) {
  ...
  FuncState new_fs;
  BlockCnt bl;
  ParamDefaultInfo defaults[MAX_PARAM_DEFAULTS];
  int ndefaults = 0;
  ...
  parlist_with_defaults(ls, defaults, &ndefaults);
  checknext(ls, ')');
  /* 生成参数默认值检查代码 */
  if (ndefaults > 0) {
    gen_param_defaults(ls, defaults, ndefaults);
  }
  statlist(ls);
...
static void suffixedexp (LexState *ls, expdesc *v) {
  /* suffixedexp ->
       primaryexp { '.' NAME | '[' exp ']' | ':' NAME funcargs | funcargs | '|>' suffixedexp } */
  FuncState *fs = ls->fs;
  int line = ls->linenumber;
  primaryexp(ls, v);
  for (;;) {
