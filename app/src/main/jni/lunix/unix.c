...
#if HAVE_SIGTIMEDWAIT && !defined ANDROID
	siginfo_t info;
	int signo;
...
#if HAVE_GETIFADDRS && !defined ANDROID
...
#if HAVE_FMEMOPEN && !defined ANDROID
...
#elif HAVE_PROGRAM_INVOCATION_SHORT_NAME && !defined ANDROID
...
#if defined ANDROID
	return unixL_pusherror(L, ENOTSUP, "lockf", "~$#");
#else
	int fd = unixL_checkfileno(L, 1);
...
#if HAVE_MKFIFOAT && !defined ANDROID
...
static void st_pushfield(lua_State *L, const struct stat *st, enum st_field type) {
	...
	case STF_SIZE:
		...
		break;
	case STF_ATIME:
#if HAVE_STRUCT_STAT_ST_ATIMESPEC && !defined ANDROID
		lua_pushnumber(L, u_ts2f(&st->st_atimespec));
#elif HAVE_STRUCT_STAT_ST_ATIM && !defined ANDROID
		...
		break;
	case STF_MTIME:
#if HAVE_STRUCT_STAT_ST_MTIMESPEC && !defined ANDROID
		lua_pushnumber(L, u_ts2f(&st->st_mtimespec));
#elif HAVE_STRUCT_STAT_ST_MTIM && !defined ANDROID
		...
		break;
	case STF_CTIME:
#if HAVE_STRUCT_STAT_ST_CTIMESPEC && !defined ANDROID
		lua_pushnumber(L, u_ts2f(&st->st_ctimespec));
#elif HAVE_STRUCT_STAT_ST_CTIM && !defined ANDROID
