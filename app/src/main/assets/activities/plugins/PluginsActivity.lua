    ...
    for index = 0, #fileList - 1 do
      ...
      if file.isDirectory() then
        ...
        if success then
          ...
          -- 检查兼容性
          local supports = config.LuaConfig
          if supports then
            local versionConfig = supports["Config"]
        ...
        if UiUtil.isNightMode() then
          -- 夜间模式：先检查插件夜间图标，再检查默认夜间图标，最后检查插件日间图标
          local nightDefaultPath = activity.getLuaDir("icon_night.png")
          
          if File(icon_night).isFile() then
            icon = icon_night
          elseif File(nightDefaultPath).isFile() then
            icon = nightDefaultPath
          elseif not File(icon).isFile() then
            -- 夜间图标都不存在，使用默认日间图标
            icon = activity.getLuaDir("icon.png")
          end
        else
