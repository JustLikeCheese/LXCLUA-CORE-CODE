...
local function parseManifest(manifestPath)
  ...
  return {
    label = application.label or "LuaApp",
...
if success and message then
  ...
  task(100,function()
    ...
    -- 处理图标
    if isDark() then
      -- 夜间模式：先检查插件夜间图标，再检查默认夜间图标
      local nightIconPath = path .. "/icon-night.png"
      local nightDefaultPath = activity.getLuaDir("icon_night.png")
      
      if FileUtil.isFile(nightIconPath) then
        GlideUtil.set(nightIconPath, icon)
      elseif FileUtil.isFile(nightDefaultPath) then
        GlideUtil.set(nightDefaultPath, icon)
      else
        -- 夜间图标都不存在，使用日间图标
        local dayIconPath = FileUtil.isFile(path .. "/icon.png") and path .. "/icon.png" or activity.getLuaDir("icon.png")
        GlideUtil.set(dayIconPath, icon)
      end
    else
      -- 日间模式：先检查插件日间图标，再检查默认日间图标
      local iconPath = FileUtil.isFile(path .. "/icon.png") and path .. "/icon.png" or activity.getLuaDir("icon.png")
      GlideUtil.set(iconPath, icon)
    end
