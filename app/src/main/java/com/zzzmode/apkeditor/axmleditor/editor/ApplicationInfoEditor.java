 ...
 * 修改<application>节点属性
 *
 * <p>ApplicationInfoEditor applicationInfoEditor = new ApplicationInfoEditor(doc);
 * applicationInfoEditor.setEditorInfo(new ApplicationInfoEditor.EditorInfo("app_name", false));
 * //设置app name 和是否开启debuggable applicationInfoEditor.commit();
 *
    ...
    if (node != null) {
      ...
      if (editorInfo.debuggableHasEditor) {
        if (editorInfo.debuggableNewEditor) {
          BTagNode.Attribute debug_attr =
              new BTagNode.Attribute(
                  namespace, editorInfo.debuggable_Index, TypedValue.TYPE_STRING);
