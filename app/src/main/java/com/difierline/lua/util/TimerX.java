...
public class TimerX
 {
    ...
    public TimerX(String name, boolean isDaemon) {
        if (name == null) {
            throw new NullPointerException("名称不能为null");
        }
    ...
    public void schedule(TimerTaskX task, Date when) {
        if (when.getTime() < 0) {
            throw new IllegalArgumentException("无效的时间：执行时间不能为负值");
        }
    ...
    public void schedule(TimerTaskX task, long delay, long period) {
        if (delay < 0 || period <= 0) {
            throw new IllegalArgumentException("无效的时间参数：延迟不能为负值，且周期必须大于0");
        }
    ...
    private void scheduleImpl(TimerTaskX task, long delay, long period, boolean fixed) {
        synchronized (impl) {
            if (impl.cancelled) {
                throw new IllegalStateException("Timer已被取消");
            }

            long when = delay + System.currentTimeMillis();

            if (when < 0) {
                throw new IllegalArgumentException("无效的TimerTask延迟时间: " + when);
            }

            synchronized (task.lock) {
                if (task.isScheduled()) {
                    throw new IllegalStateException("TimerTask已被调度");
                }

                if (task.cancelled) {
                    throw new IllegalStateException("TimerTask已被取消");
                }
