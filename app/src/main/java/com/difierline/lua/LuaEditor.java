...
public class LuaEditor extends FreeScrollingTextField {
    ...
    private String fontDir = "/sdcard/XCLUA/fonts";
    ...
    public void setHighlightHexColorsEnabled(boolean enabled) {
        super.hexColorHighlightEnabled = enabled;  // 设置父类的变量
        // 设置词法分析器的变量
        if (Lexer.getLanguage() instanceof LanguageLua) {
            ((LanguageLua) Lexer.getLanguage()).setHighlightHexColorsEnabled(enabled);
        }
        invalidate(); // 重绘视图
    }
    
    public void setCompletionCaseSensitive(boolean caseSensitive) {
        _autoCompletePanel.setCaseSensitive(caseSensitive);
    }
...
public void startFindMode() {
    ...
    startActionMode(new ActionMode.Callback() {
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            ...
            // ✅ 添加搜索按钮
            menu.add(0, 2, 0, "搜索")
                .setIcon(com.google.android.material.R.drawable.ic_search_black_24)
