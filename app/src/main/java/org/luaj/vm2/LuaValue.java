...
public class LuaValue extends Varargs {
    ...
    protected LuaValue argerror(String expected) {
        throw new LuaError("参数类型错误：预期 " + expected + "，实际得到 " + typename());
    }


    public static LuaValue argerror(int iarg, String msg) {
        throw new LuaError("参数#" + iarg + " 错误: " + msg);
    }


    protected LuaValue typerror(String expected) {
        throw new LuaError("类型错误：预期 " + expected + "，实际得到 " + typename());
    }


    protected LuaValue unimplemented(String fun) {
        throw new LuaError("操作未实现：'" + fun + "' 不支持 " + typename() + " 类型");
    }


    protected LuaValue illegal(String op, String typename) {
        throw new LuaError("非法操作：'" + op + "' 不能用于 " + typename + " 类型");
    }


    protected LuaValue lenerror() {
        throw new LuaError("无法获取长度：不能对 " + typename() + " 类型使用 # 运算符");
    }


    protected LuaValue aritherror() {
        throw new LuaError("算术运算错误：无法对 " + typename() + " 类型执行算术运算");
    }


    protected LuaValue aritherror(String fun) {
        throw new LuaError("算术运算错误：无法对 " + typename() + " 类型执行 " + fun + " 运算");
    }


    protected LuaValue compareerror(String rhs) {
        throw new LuaError("比较错误：无法比较 " + typename() + " 和 " + rhs + " 类型");
    }


    protected LuaValue compareerror(LuaValue rhs) {
        throw new LuaError("比较错误：无法比较 " + typename() + " 和 " + rhs.typename() + " 类型");
    }
    ...
    public LuaValue neg() {
        return checkmetatag(UNM, "尝试算术运算 ").call(this);
    }


    public LuaValue len() {
        return checkmetatag(LEN, "尝试获取长度 ").call(this);
    }
    ...
    protected LuaValue arithmt(LuaValue tag, LuaValue op2) {
        ...
        if (h.isnil()) {
            h = op2.metatag(tag);
            if (h.isnil())
                error("算术运算错误：无法对 " + typename() + " 和 " + op2.typename() + " 类型执行 " + tag + " 运算");
        }
    ...
    protected LuaValue arithmtwith(LuaValue tag, double op1) {
        ...
        if (h.isnil())
            error("算术运算错误：无法对 " + typename() + " 类型和数字执行 " + tag + " 运算");
    ...
    public LuaValue comparemt(LuaValue tag, LuaValue op1) {
        ...
        return error("比较错误：无法对 " + typename() + " 和 " + op1.typename() + " 类型执行比较操作");
    }


    public int strcmp(LuaValue rhs) {
        error("比较错误：无法比较 " + typename() + " 类型");
        return 0;
    }
